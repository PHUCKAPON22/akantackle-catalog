...
Main catalogs and one level of sub catalogs live in categories. A main catalog includes its children. The dashboard creates and deletes both levels, uploads products into either level, publishes/unpublishes selections, and deletes selected products plus their normalized images. Before catalog deletion, Admin counts every assigned product, including Pending and hidden rows, and counts direct children. Only an empty catalog without sub catalogs reaches confirmation. Database RESTRICT foreign keys protect against concurrent inserts between the check and deletion. A deleted active catalog automatically falls back to All catalogs in Admin and public browsing. Product selection covers the current page (48 products).
